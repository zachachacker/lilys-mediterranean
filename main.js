...
(() => {
  ...
  if (tabs && body) {
    MENU.forEach((cat, i) => {
      ...
      cat.items.forEach(([n, d, p, tag]) => {
        rows += `<div class="menu-item"><span class="mi-name">${n}${tag ? `<span class="tag">${tag}</span>` : ""}</span><span class="mi-price">${p}</span><span class="mi-desc">${d}</span></div>`;
      });
  ...
  const HOURS = { 0: [11, 22], 1: [11, 22], 2: [11, 22], 3: null, 4: [11, 22], 5: [11, 22], 6: [11, 22] };
  const now = new Date();
  ...
  if (mount) {
    let label;
    if (open) {
      label = `Open now · until ${today[1] - 12}pm`;
    } else if (today && hour < today[0]) {
      label = `Opens ${today[0]}am today`;
    } else {
      // find next open day
      let n = 1; while (n <= 7 && !HOURS[(day + n) % 7]) n++;
      const names = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
      label = `Closed · opens ${names[(day + n) % 7]} 11am`;
    }
